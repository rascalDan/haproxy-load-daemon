...
namespace haproxy_load {
	...
	{
		out << std::format(
				"[ base: {}, minimum: {}, limit: {}, crit: {} ]", agent.base, agent.minimum, agent.limit, agent.crit);
		return out;
	}
}

static constexpr haproxy_load::Agent single {.25, 1.5, 1.8, 1.9}, multi {0.2, 6.0, 8.0, 9.0},
		large {0.1, 24.0, 45.0, 47.0};
...
BOOST_DATA_TEST_CASE(response,
		boost::unit_test::data::make<responses>({
				...
				{1.5, single, "up ready 1%"},
				{1.79, single, "up ready 1%"},
